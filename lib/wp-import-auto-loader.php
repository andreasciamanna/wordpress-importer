<?php
if ( ! class_exists( 'WP_Import_Auto_Loader' ) ) {
	/**
	 * Class WP_Import_Auto_Loader
	 ...
	 * Call `WP_Import_Auto_Loader::get_instance()->register( dirname( __FILE__ ) . '/' );` from the main plugin file to register the auto loading for each plugin.
	 * Call `WP_Import_Auto_Loader::get_instance()->register( $class, $file );` to register "known" class files (in case they don't match the auto loading convention, but you want to load them automatically anyway.
	...
	class WP_Import_Auto_Loader {
		...
		/**
		 * @return WP_Import_Auto_Loader
		 */
		public static function get_instance() {
			if ( null === self::$instance ) {
				self::$instance = new WP_Import_Auto_Loader();
			}
